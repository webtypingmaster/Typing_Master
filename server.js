...
const ADMIN_USERNAME = process.env.ADMIN_USERNAME || 'TypingMaster';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'AdminTypingMaster1345';
const JWT_SECRET     = process.env.JWT_SECRET     || 'typingmaster-admin-jwt-2024';

// ─── MongoDB Connection ───────────────────────────────────────────────────────
const MONGO_URI = process.env.MONGO_URI ||
  'mongodb+srv://Ghazi:Ghazi-1245@cluster0.iix2sd1.mongodb.net/typingmaster?retryWrites=true&w=majority&appName=Cluster0';
...
app.use(session({
  secret: process.env.SESSION_SECRET || 'typing-master-secret-2024',
  resave: false,
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    // Token directly return karo (real app mein email bhejna hoga)
    res.json({
      success: true,
      message: 'Reset token generate ho gaya.',
      token: token,  // Frontend pe show hoga (demo mode)
...
app.get('/api/invite-stats', requireAuth, async (req, res) => {
  try {
    ...
    const invitedList = await Promise.all(invited.map(async (u) => {
      const results = await Result.find({ userId: u._id }).lean();
      const levelsCompleted = new Set(results.map(r => r.level)).size;
      const bestWpm = results.length ? Math.max(...results.map(r => r.wpm)) : 0;
      return {
        name: u.name,
...
app.get('/api/admin/stats', adminAuth, async (req, res) => {
  try {
    ...
    const completedAll = Object.values(byUser).filter(s => s.size >= 10).length;
...
app.get('/api/admin/users', adminAuth, async (req, res) => {
  try {
    ...
    const data  = await Promise.all(users.map(async u => {
      const userResults     = await Result.find({ userId: u._id }).lean();
      const bestWpm         = userResults.length ? Math.max(...userResults.map(r => r.wpm)) : 0;
      const levelsCompleted = new Set(userResults.map(r => r.level)).size;
      return {
        id: u._id, name: u.name, email: u.email,
        isAdmin: u.isAdmin || false,
        createdAt: u.createdAt,
        totalTests: userResults.length, levelsCompleted, bestWpm
      };
    }));
